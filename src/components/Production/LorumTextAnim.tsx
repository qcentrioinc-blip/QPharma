...
const LorumTextAnim = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-white text-black flex items-center justify-center p-6 md:p-12 lg:p-16">
      <div className="max-w-7xl w-full grid grid-cols-1 lg:grid-cols-2 items-center gap-10 md:gap-16">
              ...
              <div key={rowIndex} className="flex flex-row">
                {columns.map((colIndex) => (
                  <div
                    key={colIndex}
                    className="
                      w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 xl:w-24 xl:h-24 font-serif
                      font-extrabold flex items-center justify-center 
                      text-4xl sm:text-6xl md:text-7xl xl:text-8xl tracking-tighter
                    "
                  >
                    <span
                      className={`transform -rotate-90 inline-block transition-transform duration-500 hover:scale-110 ${colIndex === 0 ? "text-black" : "text-transparent"
                        }`}
                      style={
                        colIndex > 0
